...
def main(N, CHR, infile, outfile):
    ...
    rd = rda.RDA()
    rd.build_hmm('wrightfisher', 'dunif', np.unique(np.hstack((
        np.linspace(-1, 0, 100),
        np.linspace(0, 1, 100)))),
                 np.array([N]), np.array([0, 1, 2, 3, 4, 5]), nop = 129)
...
---------------------------------------------------------------------------''',
    ...
    parser.add_argument('--CHR', dest = 'CHR', action = 'store', type = int,
                        default = -1,
                        help = "Analyze only the corresponding CHR in infile")
    ...
    main(args.N, args.CHR, args.inf, args.ouf)
